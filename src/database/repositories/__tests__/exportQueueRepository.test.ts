...
describe('exportQueueRepository', () => {
  ...
  describe('updateExportQueueItem', () => {
    ...
    it('should update multiple fields', async () => {
      ...
      expect(mockDb.executeSql).toHaveBeenCalledWith(
        expect.stringMatching(
          /status = \?.*last_error = \?.*drive_file_id = \?.*uploaded_at = \?/,
        ),
    ...
    it('should handle null values in updates', async () => {
      ...
      expect(mockDb.executeSql).toHaveBeenCalledWith(
        expect.stringMatching(
          /last_error = \?.*drive_file_id = \?.*uploaded_at = \?/,
        ),
    ...
    it('should throw error if item not found', async () => {
      ...
      await expect(
        updateExportQueueItem(mockDb, 'non-existent', updates),
      ).rejects.toThrow('Export queue item non-existent not found');
    });
  ...
  describe('removeExportQueueItem', () => {
    ...
    it('should throw error if item not found', async () => {
      ...
      await expect(
        removeExportQueueItem(mockDb, 'non-existent'),
      ).rejects.toThrow('Export queue item non-existent not found');
    });
  });
