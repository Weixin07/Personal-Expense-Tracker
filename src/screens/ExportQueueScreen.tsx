...
const ExportQueueScreen: React.FC = () => {
  ...
  const handleClearCompleted = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        'Clear failed',
        error instanceof Error ? error.message : 'Unable to clear exports.',
      );
    } finally {
      setIsClearing(false);
    }
  ...
  const handleUploadNow = useCallback(async () => {
    ...
    try {
      ...
      if (!result) {
        Alert.alert(
          'Upload in progress',
          'Another upload is already running. Please try again shortly.',
        );
        return;
      }
      if (result.requiresAuth) {
        Alert.alert(
          'Sign in required',
          'Please sign in to Google Drive and retry the upload.',
        );
        return;
      }
      if (result.attempted === 0) {
        Alert.alert(
          'Nothing to upload',
          'No pending exports are available for upload.',
        );
        return;
      }
      if (result.failed > 0) {
  ...
  const keyExtractor = useCallback(
    (item: (typeof exportQueue)[number]) => item.id,
    [],
  );
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            ...
            {pendingCount > 5 ? (
              <View style={styles.banner}>
                <Text variant="bodySmall">
                  There are more than five exports waiting to upload. Connect to
                  the internet and upload soon.
                </Text>
              </View>
            ) : null}
            ...
            <Text variant="bodySmall" style={styles.helperText}>
              Exports are stored locally and uploaded when online. Retry failed
              exports or remove entries that are no longer needed.
            </Text>
            <Text variant="bodySmall" style={styles.helperText}>
