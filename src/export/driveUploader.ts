...
const folderExists = async (
  ...
  const response = await fetch(
    `${DRIVE_FILES_ENDPOINT}/${folderId}?fields=id,trashed`,
    {
      method: 'GET',
      headers: {
...
const uploadFileToDrive = async (
  ...
  if (!data.id) {
    throw new Error(
      `Google Drive upload response missing file ID for ${item.filename}.`,
    );
  }
  return { id: data.id };
};
...
export const uploadPendingExports = async (
  ...
  return withDatabase(async db => {
    ...
    for (const item of pending) {
      ...
      await dbUpdateExportQueueStatus(db, item.id, 'uploading', {
        lastError: null,
      });
      ...
      try {
        ...
        await dbUpdateExportQueueStatus(db, item.id, 'failed', {
          lastError: message,
        });
        result.failed += 1;
